...
function draw_AdjRect(thisItem, color, lineColor = "#00000", line_Width = 2) {
    ctx.beginPath()
...
function polygon(initial_theta, p_dist, data) {
    ...
    // prev_btn_cordinate.x = prev_btn_cordinate.x - 200
    // next_btn_cordinate.x = next_btn_cordinate.x + 200
    //buildPrevNextBtn(prev_btn_cordinate, ctx, "#FFFFFF", "#000000", 0.7, "PREV")
    //buildPrevNextBtn(next_btn_cordinate, ctx, "#FFFFFF", "#000000", 0.7, "NEXT")
    ...
    adjacent_plygn_cordinates.forEach((item, i) => {
        draw_AdjRect(item.layer2_ply, "#000000", "#FFFFFF")
    })
    // review graph
...
function DrawGraph(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        //Adject circle arc
        DrawCircle(this_center_x, this_center_y, attach_circle_r + adjt_circle_offset, line_cordinates[i].theta - (Math.PI / 2) + Math.PI, line_cordinates[i].theta + (Math.PI / 2) + Math.PI, 10, false, true)
...
function doPgPrevFun() {
    if ((curr_pg - 1) * total_tag_Per_pg > 0) {
        ...
        thisData = []
        console.log((curr_pg - 1) * total_tag_Per_pg - 1, Math.max(((curr_pg - 2) * total_tag_Per_pg), 0))
