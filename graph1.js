...
function draw_AdjRect(thisItem,color) {
	ctx.beginPath()
	ctx.fillStyle = color;
...
function plotDescription(thispoly,thisTheta) {
	let x1=thispoly[0][0]
	let y1=thispoly[0][1]
	let x2=thispoly[1][0]
	let y2=thispoly[1][1]
...
function polygon(ctx, x, y, radius, sides,initial_theta,p_dist) {
  ...
  //console.log("yes")
	...
	for (var i = 0; i < adjacent_plygn.length; i++) {
	  ...
	  var y1=adjacent_plygn[i][0].y
		...
		adjacent_plygn_cordinates.forEach((item,i)=>{
			...
			draw_AdjRect(item.layer2_ply,"#000000")
	...
	if(onclick_val_index || onclick_val_index ===0){
  	...
  	draw_AdjRect(thisPoly,"#FFFFFF")


  	plotDescription(thisPoly,thisTheta)
